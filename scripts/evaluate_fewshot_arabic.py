#!/usr/bin/env python3
"""
Evaluate few-shot translation on Tunisian Arabic test set.
Runs translations with specified num_examples.
...
DATASET_NAME = "predictionguard/arabic_acl_corpus"
...
# Configuration - using Modern Standard Arabic as pivot
CONFIG = {
    "pivot": "msa",
    "source": "en",
    "target": "tn",
...
def flatten_translation_df(df):
    """Flatten the 'translation' dict column into separate language columns."""
    if 'translation' in df.columns:
        # Extract language columns from translation dict
        for lang in ['en', 'msa', 'tn', 'eg']:
            df[lang] = df['translation'].apply(lambda x: x.get(lang, '') if isinstance(x, dict) else '')
    return df


def load_vector_db(df, db_name="arabic_translations", table_name="translations_tn", force_recreate=False):
    ...
    # Flatten if needed
    df = flatten_translation_df(df)
    
    # Create vector DB
    required_cols = ['en', 'msa', 'tn']
...
def build_prompt_messages(source_text, config, examples_df=None):
    ...
    language_constraint = (
        f"CRITICAL: You are translating to Tunisian Arabic (Derja). "
        f"Output ONLY Tunisian Arabic text. Never output Modern Standard Arabic or Egyptian Arabic.\n\n"
    )
            ...
            if config['pivot']:
                if first_example:
                    user_content = (
                        ...
                        f"Translation (Modern Standard Arabic): {row[config['pivot']]}\n"
                        f"Post-edited (Tunisian Arabic):"
                    )
                    first_example = False
                else:
                    user_content = (
    ...
    if config['pivot']:
        if examples_df is None or len(examples_df) == 0:
            user_content = (
                ...
                f"Translation (Modern Standard Arabic):\n"
                f"Post-edited (Tunisian Arabic):"
            )
        else:
            user_content = (
...
def translate_with_fewshot(source_text, model, tokenizer, db, embed_model, table_name, config, num_examples):
    ...
    stop_phrases = [
        ...
        "\nPost-edited (Tunisian Arabic):", "again!", "again"
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate few-shot translation on Tunisian Arabic test set")
    ...
    parser.add_argument("--config", choices=["1", "2"], default="1", help="Configuration: 1=MSA pivot, 2=No pivot")
    ...
    parser.add_argument("--force-recreate-db", action="store_true", help="Force recreate vector database")
    ...
    # Flatten translation dict into separate columns
    test_df = flatten_translation_df(test_df)
    train_df = flatten_translation_df(train_df)
    print(f"Columns after flattening: {list(test_df.columns)}")
    ...
    db, embed_model, table_name = load_vector_db(train_df, force_recreate=args.force_recreate_db)
    
    # Update config based on choice
    if args.config == "1":
        config = {"pivot": "msa", "source": "en", "target": "tn", "name": "MSA as pivot"}
    else:
        config = {"pivot": None, "source": "en", "target": "tn", "name": "No pivot"}
    ...
    for idx, row in tqdm(test_df.iterrows(), total=len(test_df), desc=f"Translating (k={num_examples})"):
        ...
        try:
            ...
            result = {
                ...
                "prompt": prompt,
                "examples_used": json.dumps(examples_used, ensure_ascii=False)
            }
    ...
    # Save results
    if not results:
        print("\nERROR: No results were generated. All examples failed.")
        print("Try running with --force-recreate-db to rebuild the vector database.")
        return
    
    results_df = pd.DataFrame(results)
    model_short = args.model.split("/")[-1]
    output_file = os.path.join(args.output_dir, f"arabic_test_{model_short}_k{num_examples}.csv")
    ...
    metrics_file = os.path.join(args.output_dir, f"arabic_test_{model_short}_k{num_examples}_metrics.json")
